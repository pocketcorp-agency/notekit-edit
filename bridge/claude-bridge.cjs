...
var qrcode = function(typeNumber, errorCorrectionLevel) {
  ...
  _this.createSvgTag = function(cellSize, margin, alt, title) {
    ...
    qrSvg += !opts2.scalable ? ' width="' + size + 'px" height="' + size + 'px"' : "";
...
function qrMatrix(text) {
  ...
  return Array.from({ length: n }, (_, r) => Array.from({ length: n }, (_2, c) => qr.isDark(r, c)));
}
function renderQr(text, { quiet = 2, colour = true } = {}) {
...
// src/setup-link.ts
var SETUP_LINK_ACTION = "notekit-edit";
var SETUP_LINK_VERSION = "1";
function buildSetupLink(cfg) {
  const params = [
    ...
    ["effort", cfg.effort],
    ["headers", cfg.headers],
    ["default", cfg.makeDefault === void 0 ? void 0 : String(cfg.makeDefault)],
    ["vault", cfg.vault]
  ];
  const query = params.filter((p) => typeof p[1] === "string" && p[1] !== "").map(([k, v]) => `${k}=${encodeURIComponent(v)}`).join("&");
...
var MODEL = opts.backend === "codex" ? "codex" : opts.backend === "acp" ? "acp" : "claude";
var AGENT_NAME = opts.backend === "codex" ? "Codex bridge" : opts.backend === "acp" ? "ACP bridge" : "Claude Code bridge";
function candidateBaseUrls() {
  ...
  const rank = (ip) => /^192\.168\./.test(ip) ? 0 : /^10\./.test(ip) ? 1 : /^172\.(1[6-9]|2\d|3[01])\./.test(ip) ? 2 : /^100\.(6[4-9]|[7-9]\d|1[01]\d|12[0-7])\./.test(ip) ? 3 : 4;
  const ips = [];
