...
try {
  ...
  var agents = [["claude-code", "Claude Code"], ["codex", "Codex"], ["ollama", "Ollama"], ["lm-studio", "LM Studio"], ["hermes", "Hermes Agent"], ["acp", "ACP agents"]];
  var folder = "/Users/maidmor/Desktop/obsidian-ai/docs/trailer/ae/media/logos/";
  for (var i = 0; i < agents.length; i++) {
    var id = agents[i][0], label = agents[i][1]; var x = 960 + (i - (agents.length - 1) / 2) * 190, y = 650;
    ...
    else item.replace(new File(folder + id + ".png"));   // pick up recoloured tiles
